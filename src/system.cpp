...
// 3DS Specific
    gAudioBuffer0 = (UBYTE*)linearAlloc(HANDY_AUDIO_BUFFER_SIZE);
    gAudioBuffer1 = (UBYTE*)linearAlloc(HANDY_AUDIO_BUFFER_SIZE);
    gAudioBuffer2 = (UBYTE*)linearAlloc(HANDY_AUDIO_BUFFER_SIZE);
    gAudioBuffer3 = (UBYTE*)linearAlloc(HANDY_AUDIO_BUFFER_SIZE);
//
...
// 3DS specific
   if (gAudioBuffer0) linearFree(gAudioBuffer0);
   if (gAudioBuffer1) linearFree(gAudioBuffer1);
   if (gAudioBuffer2) linearFree(gAudioBuffer2);
   if (gAudioBuffer3) linearFree(gAudioBuffer3);
//
...
bool CSystem::ContextSave(char *context)
{
	...
	if(!fwrite(gAudioBuffer0,sizeof(UBYTE),HANDY_AUDIO_BUFFER_SIZE,fp)) status=0; // !!!!! NOP90 - to not change the SSL format wi will lose 0,25s of audio on 3 channels at resume 
...
bool CSystem::ContextLoad(char *context)
{
	...
	if(strcmp(teststr,LSS_VERSION)==0 || strcmp(teststr,LSS_VERSION_OLD)==0)
	{
		...
		if(!lss_read(gAudioBuffer0,sizeof(UBYTE),HANDY_AUDIO_BUFFER_SIZE,fp)) status=0;  // NOP90 - see remark on save context
